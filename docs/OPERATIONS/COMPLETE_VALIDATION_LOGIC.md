...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 989-996: Checks if title contains age pattern using `has_age_in_text()` helper function (lines 943-948). Regex patterns: `ages?\s*\d{1,2}`, `students?\s*\d{1,2}`, `\d{1,2}\s*[-–+]`, `\d{1,2}\s*to\s*\d{1,2}`. Applies to ALL event types.

**2. Temporary Overrides (Dismissed Errors):** 
- Stored in `acknowledged_errors` JSONB field in events table
- Added when user clicks "Accept Exception" in DismissRuleModal
- Persists on that specific event only  
- Does NOT prevent validation from running
- Only hides the error in UI after it's generated

**3. Permanent Rules (gym_valid_values):** Not applicable - this check does not use gym_valid_values table.

**4. All Combinations:**
- No rule: Error generated if title missing age pattern
- Only embedded rule: Always generates error when age pattern not found
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 999-1006: Checks if title contains date pattern using `has_date_in_text()` helper function (lines 951-958). Regex patterns: month names (jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec), date formats (\d{1,2}/\d{1,2}|\d{1,2}(st|nd|rd|th)). Applies to ALL event types.
...
**4. All Combinations:**
- No rule: Error generated if title missing date pattern
- Only embedded rule: Always generates error when date pattern not found
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1035-1042: Checks if title contains program type keyword using `has_program_type_in_text()` (lines 1011-1033). Hardcoded keywords: KNO=['kids night out','kid night out','kno','night out','parents night out','ninja night out'], CLINIC=['clinic'], OPEN GYM=['open gym'], CAMP=['camp']. Applies to ALL event types.
...
**3. Permanent Rules (gym_valid_values):** YES - uses program_synonym rules. Lines 1016-1021: Function checks gym_valid_values for program_synonym rules FIRST before checking hardcoded keywords. If synonym rule exists where keyword matches text and label matches event_type, returns True (passes check). Example: If gym has rule {value:'pno', label:'KIDS NIGHT OUT'}, then "pno" in title passes for KNO event.

**4. All Combinations:**
- No rule: Error generated if hardcoded keyword not found in title
- Only embedded rule: Checks only hardcoded keywords (kno, clinic, open gym, camp)
- Temp override only: Error generated by embedded rules → hidden in UI by acknowledged_errors
- Permanent rule only: Error NOT generated because program_synonym allows alternate keyword
- Both temp + permanent: Error NOT generated (permanent takes precedence)
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1048-1055: Checks if description contains age pattern using has_age_in_text() helper. Applies to ALL event types. Only runs if description exists.
...
**4. All Combinations:**
- No rule: Error generated if description missing age pattern
- Only embedded rule: Always generates error when pattern not found
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1058-1065: Checks if description contains date/time pattern using has_date_in_text() and has_time_in_text() helpers. Applies to ALL event types. Only runs if description exists.
...
**4. All Combinations:**
- No rule: Error generated if description missing date/time pattern
- Only embedded rule: Always generates error when pattern not found
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1070-1077: Checks if description contains time pattern using has_time_in_text() helper. Exception for CAMP: Allows 'Full Day' or 'Half Day' instead of specific time (line 1069). Applies to ALL event types. Only runs if description exists.
...
**4. All Combinations:**
- No rule: Error generated if description missing time pattern
- Only embedded rule: Always generates error when pattern not found
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1080-1087: Checks if description contains program type keyword using `has_program_type_in_text()` which calls same logic as title check (lines 1011-1033). Hardcoded keywords by type. Applies to ALL event types. Only runs if description exists.
...
**3. Permanent Rules (gym_valid_values):** YES - uses program_synonym rules. Lines 1016-1021 in helper function: Checks gym_valid_values program_synonym rules FIRST. If synonym rule matches, returns True (passes check). Same logic as title program check.

**4. All Combinations:**
- No rule: Error generated if hardcoded keyword not found in description
- Only embedded rule: Checks only hardcoded keywords
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Error NOT generated because program_synonym allows alternate keyword
- Both temp + permanent: Error NOT generated (permanent takes precedence)
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1092-1109: Checks if description OR title contains skill keyword from hardcoded list (25 skills). Only applies to CLINIC events. Only runs if description exists. Severity is 'info' not warning/error.
...
**4. All Combinations:**
- No rule: Info message generated if no skill found in title or description
- Only embedded rule: Always generates info message when skill not found
- Temp override only: Info message generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1772-1779: Checks if description contains price pattern ($XX or $XX.XX) using regex `\$(\d+(?:\.\d{2})?)`. Applies to ALL event types. Only runs if description exists.
...
**4. All Combinations:**
- No rule: Error generated if no $ pattern found in description
- Only embedded rule: Always generates error when price not found
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 923-928: Sets description_status field based on presence of description text and has_flyer. Not added to validation_errors array. Handled separately in UI. Lines 1937-1940: Logged but not validated.
...
- Does NOT prevent validation from running
- Only hides the error in UI after it's generated
- This status field cannot be dismissed as it's not a validation error

**3. Permanent Rules (gym_valid_values):** Not applicable - this status check does not use gym_valid_values table.

**4. All Combinations:**
- No rule: Status set based on description/flyer presence
- Only embedded rule: Always sets status field
- Temp override only: Not applicable (not a validation error)
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1134-1141: Compares iClass endDate < startDate. Built-in universal rule, no per-gym setup. Only runs if description exists (wrapper condition line 1115). This is a manager data entry error in iClass.
...
**3. Permanent Rules (gym_valid_values):** Not applicable - this check does not use gym_valid_values table. This is a built-in data integrity rule.

**4. All Combinations:**
- No rule: Error generated if end date before start date
- Only embedded rule: Always generates error when dates inverted
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1143-1203: Scans description first 200 chars for month names that don't match event dates. Built-in logic includes ALL months between startDate and endDate (lines 1152-1158). Skip patterns (lines 1181-1186): Ignores registration/promo context like "register by", "sign up", "upcoming", etc. Universal rule applies to ALL events.
...
**3. Permanent Rules (gym_valid_values):** Not applicable - this check does not use gym_valid_values table. Built-in rule with context-aware skip patterns.

**4. All Combinations:**
- No rule: Error generated if wrong month found (not in valid range, not in skip context)
- Only embedded rule: Always generates error when wrong month detected
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1206-1219: Scans title for 4-digit year pattern (20XX) and compares to event year from startDate. Only checks TITLE not description (known gap). Universal rule. Only runs if description exists (wrapper condition).
...
**4. All Combinations:**
- No rule: Error generated if title year doesn't match event year
- Only embedded rule: Always generates error when year mismatch found
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1221-1298: Extracts times from iClass time_str and title, converts to 24-hour format, checks for EXACT HOUR match. Uses `check_times_in_text()` helper (lines 1240-1281) with pre-cleaning to remove false positives ($62 a day, Ages 4-13 patterns). Only runs if time_str exists and description exists (wrapper).
...
**3. Permanent Rules (gym_valid_values):** YES - uses time rules. Lines 1289-1300: After detecting mismatch, checks if the found time appears in gym_valid_values 'time' rules (lines 1284-1285 get rules). If time is in extra_time_values for this gym, does NOT generate error and prints "[OK] Time {time} is a valid extra time for {gym_id}". Example: If gym has rule {value:'8:30 AM'}, then "8:30 AM" in title won't trigger error.

**4. All Combinations:**
- No rule: Error generated if title time doesn't match iClass time
- Only embedded rule: Checks only exact hour match against iClass time
- Temp override only: Error generated by embedded rules → hidden in UI by acknowledged_errors
- Permanent rule only: Error NOT generated because time rule allows this specific time
- Both temp + permanent: Error NOT generated (permanent takes precedence)
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1302-1315: Extracts times from iClass time_str and description, converts to 24-hour format, checks for EXACT HOUR match. Uses `check_times_in_text()` helper (lines 1240-1281) with pre-cleaning to remove false positives ($62 a day, Ages 4-13 patterns). Only runs if time_str exists and description exists (wrapper).
...
**3. Permanent Rules (gym_valid_values):** YES - uses time rules. Lines 1304-1315: After detecting mismatch, checks if the found time appears in gym_valid_values 'time' rules (lines 1284-1285 get rules). If time is in extra_time_values for this gym, does NOT generate error and prints "[OK] Time {time} is a valid extra time for {gym_id}". Example: If gym has rule {value:'8:30 AM'}, then "8:30 AM" in description won't trigger error.

**4. All Combinations:**
- No rule: Error generated if description time doesn't match iClass time
- Only embedded rule: Checks only exact hour match against iClass time
- Temp override only: Error generated by embedded rules → hidden in UI by acknowledged_errors
- Permanent rule only: Error NOT generated because time rule allows this specific time
- Both temp + permanent: Error NOT generated (permanent takes precedence)
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1317-1346: Extracts minimum age from iClass vs Title using `extract_min_age()` helper (lines 1321-1331), compares for exact match. Only checks if BOTH ages exist (None values skip check). Only validates MIN age, not max (line 1370 note). Only runs if description exists (wrapper).
...
**4. All Combinations:**
- No rule: Error generated if ages don't match
- Only embedded rule: Always generates error when ages mismatch
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1348-1357: Extracts minimum age from iClass vs Description using `extract_min_age()` helper (lines 1321-1331), compares for exact match. Only checks if BOTH ages exist (None values skip check). Only validates MIN age, not max (line 1370 note). Only runs if description exists (wrapper).
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1359-1368: Extracts minimum age from Title vs Description using `extract_min_age()` helper (lines 1321-1331), compares for exact match. Only checks if BOTH ages exist (None values skip check). Only validates MIN age, not max (line 1370 note). Only runs if description exists (wrapper).
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1372-1401: Calculates day_of_week from startDate, scans description first 200 chars for different day names. Pre-cleaning (lines 1385-1389): Removes day range patterns like "Monday-Friday", "Mon-Fri", "(Monday-Friday)" before checking. Skipped for CAMP events (line 1374). Only runs if day_of_week exists and description exists.
...
**4. All Combinations:**
- No rule: Error generated if wrong day found (after removing range patterns)
- Only embedded rule: Always generates error when day mismatch detected
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1429-1436: Builds keyword lists for each program type (lines 1407-1422), checks if title contains wrong program keywords while not containing correct ones. Logic: event_type == 'KNO' AND title_has_clinic AND NOT title_has_kno. Only runs if description exists (wrapper).
...
**3. Permanent Rules (gym_valid_values):** YES - uses program_synonym rules. Lines 1411-1422: Adds synonym keywords from gym_valid_values to the keyword lists before checking. If gym has synonym that maps to correct type, title passes check. Example: If gym has {value:'ninja night', label:'OPEN GYM'}, then "ninja night" in title passes for OPEN GYM event.

**4. All Combinations:**
- No rule: Error generated if title has wrong program keyword without correct one
- Only embedded rule: Checks only hardcoded keywords
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Error NOT generated if synonym defines alternate valid keyword
- Both temp + permanent: Error NOT generated (permanent takes precedence)
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1438-1445: Builds keyword lists for each program type (lines 1407-1422), checks if title contains wrong program keywords while not containing correct ones. Logic: event_type == 'KNO' AND title_has_open_gym AND NOT title_has_kno. Only runs if description exists (wrapper).
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1447-1454: Builds keyword lists for each program type (lines 1407-1422), checks if title contains wrong program keywords while not containing correct ones. Logic: event_type == 'CLINIC' AND title_has_kno AND NOT title_has_clinic. Only runs if description exists (wrapper).
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1456-1463: Builds keyword lists for each program type (lines 1407-1422), checks if title contains wrong program keywords while not containing correct ones. Logic: event_type == 'CLINIC' AND title_has_open_gym AND NOT title_has_clinic. Only runs if description exists (wrapper).
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1465-1472: Builds keyword lists for each program type (lines 1407-1422), checks if title contains wrong program keywords while not containing correct ones. Logic: event_type == 'OPEN GYM' AND title_has_kno AND NOT title_has_open_gym. Only runs if description exists (wrapper).
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1474-1481: Builds keyword lists for each program type (lines 1407-1422), checks if title contains wrong program keywords while not containing correct ones. Logic: event_type == 'OPEN GYM' AND title_has_clinic AND NOT title_has_open_gym. Only runs if description exists (wrapper).
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1484-1500: For KNO events, checks description for 'kids night out' (any apostrophe style) or 'kno'.  Strips apostrophes for matching (line 1487). Only runs if event_type=='KIDS NIGHT OUT' and description exists.
...
**3. Permanent Rules (gym_valid_values):** Not applicable - this check does not use gym_valid_values table for description content validation.

**4. All Combinations:**
- No rule: Error generated if description missing KNO
- Only embedded rule: Always generates warning when condition met
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1502-1509: For KNO events, checks description for 'kids night out' (any apostrophe style) or 'kno'. Checks if 'clinic' appears in first 100 chars. Strips apostrophes for matching (line 1487). Only runs if event_type=='KIDS NIGHT OUT' and description exists.
...
**4. All Combinations:**
- No rule: Error generated if description says Clinic
- Only embedded rule: Always generates error when condition met
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1511-1528: For CLINIC events, checks description for 'clinic' keyword and checks for wrong program keywords (KNO/Open Gym). Checks first 100 chars for KNO/Open Gym. Strips apostrophes for KNO matching. Only runs if event_type=='CLINIC' and description exists.
...
**4. All Combinations:**
- No rule: Error generated if description missing Clinic
- Only embedded rule: Always generates warning when condition met
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1530-1537: For CLINIC events, checks description for 'clinic' keyword and checks for wrong program keywords (KNO/Open Gym). Checks first 100 chars for KNO/Open Gym. Strips apostrophes for KNO matching. Only runs if event_type=='CLINIC' and description exists.
...
**4. All Combinations:**
- No rule: Error generated if description says KNO
- Only embedded rule: Always generates error when condition met
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1539-1546: For CLINIC events, checks description for 'clinic' keyword and checks for wrong program keywords (KNO/Open Gym). Checks first 100 chars for KNO/Open Gym. Strips apostrophes for KNO matching. Only runs if event_type=='CLINIC' and description exists.
...
**4. All Combinations:**
- No rule: Error generated if description says Open Gym
- Only embedded rule: Always generates error when condition met
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1584-1612: For OPEN GYM events, checks description for 'open gym', 'play and explore the gym', or 'open to all'. ALSO checks program_synonym rules from gym_valid_values (lines 1592-1598). Only runs if event_type=='OPEN GYM' and description exists.
...
**3. Permanent Rules (gym_valid_values):** YES - uses program_synonym rules. Lines 1592-1598: After checking hardcoded keywords, function explicitly checks gym_valid_values for program_synonym rules where label='OPEN GYM'. If synonym value found in description, sets has_open_gym=True (passes check). Example: If gym has {value:'family gym time', label:'OPEN GYM'}, then "family gym time" in description passes.

**4. All Combinations:**
- No rule: Error generated if hardcoded keywords not found
- Only embedded rule: Checks only hardcoded keywords (open gym, play and explore, open to all)
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Error NOT generated because program_synonym allows alternate phrase
- Both temp + permanent: Error NOT generated (permanent takes precedence)
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1614-1621: For OPEN GYM events, checks if description first 100 chars starts with 'clinic' or contains 'clinic' in first 50. Only runs if event_type=='OPEN GYM' and description exists.
...
**4. All Combinations:**
- No rule: Error generated if description says Clinic
- Only embedded rule: Always generates error when wrong keyword detected
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1623-1630: For OPEN GYM events, checks if description first 100 chars contains 'kids night out' or 'kid night out' (after stripping apostrophes). Only runs if event_type=='OPEN GYM' and description exists.
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1636-1649: For CAMP events, checks if description first 50 chars starts with 'clinic'. Note: CAMPs can mention these as activities, only flags if description STARTS with wrong program. Only runs if event_type=='CAMP' and description exists.
...
**4. All Combinations:**
- No rule: Error generated if description starts with Clinic
- Only embedded rule: Always generates error when description starts with wrong program
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1651-1658: For CAMP events, checks if description first 50 chars starts with 'kids night out/kno'. Note: CAMPs can mention these as activities, only flags if description STARTS with wrong program. Only runs if event_type=='CAMP' and description exists.
...
**4. All Combinations:**
- No rule: Error generated if description starts with KNO
- Only embedded rule: Always generates error when description starts with wrong program
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1661-1668: Checks if iClass says CAMP but title contains 'kno' keywords. Uses title_has_kno variable built earlier (lines 1407-1422 with program_synonym support). Only runs if event_type=='CAMP' and description exists.
...
**3. Permanent Rules (gym_valid_values):** Indirectly YES - title keyword lists include program_synonym rules (lines 1411-1422). If gym defines synonym for KNO, it's checked in title.

**4. All Combinations:**
- No rule: Error generated if title has KNO keywords
- Only embedded rule: Checks hardcoded keywords only
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable (synonyms would make mismatch MORE likely not less)
- Both temp + permanent: Error generated → hidden by temp override
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1670-1677: Checks if iClass says CAMP but title contains 'clinic' keywords. Uses title_has_clinic variable built earlier (lines 1407-1422 with program_synonym support). Only runs if event_type=='CAMP' and description exists.
...
**3. Permanent Rules (gym_valid_values):** Indirectly YES - title keyword lists include program_synonym rules (lines 1411-1422). If gym defines synonym for Clinic, it's checked in title.

**4. All Combinations:**
- No rule: Error generated if title has Clinic keywords
- Only embedded rule: Checks hardcoded keywords only
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable (synonyms would make mismatch MORE likely not less)
- Both temp + permanent: Error generated → hidden by temp override
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1548-1582: For CLINIC events only, extracts skill from title and description (first 150 chars), compares if both exist and differ. Uses hardcoded skills list (25 skills). Handles 'back handspring' vs 'backhandspring' normalization. Only runs if event_type=='CLINIC' and description exists.
...
**4. All Combinations:**
- No rule: Error generated if title and description have different skills
- Only embedded rule: Always generates error when skills mismatch
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1781-1794: If price in BOTH title and description, checks if title price appears in description prices (within $1 tolerance). Extracts all prices using regex `\$(\d+(?:\.\d{2})?)`. Only runs if description exists and both title_prices and desc_prices found.
...
**3. Permanent Rules (gym_valid_values):** Not applicable - this check does not use gym_valid_values table. It validates internal consistency (title vs description), not against expected prices.

**4. All Combinations:**
- No rule: Error generated if title price not found in description prices
- Only embedded rule: Always checks for title price in description
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1706-1714: Cross-checks title vs description keywords (ANY event type). Builds title_has and desc_has variables using keyword lists (lines 1682-1704). Checks if title says 'Clinic' while description says 'KNO'. Only runs if description exists.
...
**3. Permanent Rules (gym_valid_values):** Indirectly YES - keyword lists include program_synonym rules (lines 1411-1422 for title, 1689-1695 for Open Gym). If gym defines synonyms, they're included in the has_ variables.

**4. All Combinations:**
- No rule: Error generated if title and description have conflicting program keywords
- Only embedded rule: Checks hardcoded keywords only
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not directly applicable (synonyms affect keyword detection but don't prevent mismatch)
- Both temp + permanent: Error generated → hidden by temp override
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1716-1724: Cross-checks title vs description keywords (ANY event type). Builds title_has and desc_has variables using keyword lists (lines 1682-1704). Checks if title says 'KNO' while description says 'Clinic'. Only runs if description exists.
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1726-1734: Cross-checks title vs description keywords (ANY event type). Builds title_has and desc_has variables using keyword lists (lines 1682-1704). Checks if title says 'Open Gym' while description says 'KNO'. Only runs if description exists.
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1736-1744: Cross-checks title vs description keywords (ANY event type). Builds title_has and desc_has variables using keyword lists (lines 1682-1704). Checks if title says 'KNO' while description says 'Open Gym'. Only runs if description exists.
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1746-1754: Cross-checks title vs description keywords (ANY event type). Builds title_has and desc_has variables using keyword lists (lines 1682-1704). Checks if title says 'Clinic' while description says 'Open Gym'. Only runs if description exists.
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1756-1764: Cross-checks title vs description keywords (ANY event type). Builds title_has and desc_has variables using keyword lists (lines 1682-1704). Checks if title says 'Open Gym' while description says 'Clinic'. Only runs if description exists.
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1796-1852: For CAMP events, extracts prices from title AND description, builds valid_prices list from camp_pricing table (4 price types: full_day_daily, full_day_weekly, half_day_daily, half_day_weekly). Checks each found price against valid prices with $2 tolerance (line 1842). Only runs if event_type=='CAMP' and prices found.
...
**3. Permanent Rules (gym_valid_values):** YES - uses price rules. Lines 1826-1834: After building valid_prices from camp_pricing table, function adds extra prices from gym_valid_values 'price' rules for this gym. If gym has price rule {value:'20', label:'Before Care'}, adds 20 to valid_prices list. Any price in gym_valid_values price rules passes validation.

**4. All Combinations:**
- No rule: Error generated if price doesn't match camp_pricing table prices
- Only embedded rule: Validates against camp_pricing table only (4 price types)
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Error NOT generated because price rule allows this additional valid price
- Both temp + permanent: Error NOT generated (permanent takes precedence)
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1854-1893: For CLINIC/KNO/OPEN GYM events, gets valid_prices from event_pricing table (with effective_date support), extracts ALL prices from title+description, checks if ANY valid price appears in found prices (within $1 tolerance). Only runs if event_type in these 3 types and desc_prices exist.
...
**3. Permanent Rules (gym_valid_values):** YES - uses price rules. Lines 1866-1873: After getting valid_prices from event_pricing table, function adds extra prices from gym_valid_values 'price' rules for this gym. If gym has price rule, adds that price to valid_prices. Any price in gym_valid_values price rules passes validation.

**4. All Combinations:**
- No rule: Error generated if none of the found prices match event_pricing table
- Only embedded rule: Validates against event_pricing table only
- Temp override only: Error generated → hidden in UI by acknowledged_errors
- Permanent rule only: Error NOT generated because price rule allows this additional valid price
- Both temp + permanent: Error NOT generated (permanent takes precedence)
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1900-1917: Checks if registration_end_date < today AND event_start >= today (registration closed but event upcoming). Date comparison logic. Only runs if both dates exist.
...
**3. Permanent Rules (gym_valid_values):** Not applicable - this check does not use gym_valid_values table. Based on iClass registration dates.

**4. All Combinations:**
- No rule: Warning generated if registration closed but event upcoming
- Only embedded rule: Always generates warning based on date comparison
- Temp override only: Warning generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
...
**RULE LAYER:**
**1. Embedded Rules (Hardcoded):** Lines 1919-1934: Checks if registration_start_date > today (registration hasn't opened yet). Date comparison logic. Only runs if registration_start_date exists.
...
**4. All Combinations:**
- No rule: Info message generated if registration not open yet
- Only embedded rule: Always generates info based on date comparison
- Temp override only: Info generated → hidden in UI by acknowledged_errors
- Permanent rule only: Not applicable
- Both temp + permanent: Not applicable
