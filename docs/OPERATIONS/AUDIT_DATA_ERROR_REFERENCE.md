...
**IMPORTANT:** These keywords are used in ALL KNO checks (completeness, accuracy, and cross-checks). As of March 2026, all locations are synced.

- **KNO**: kids night out, kid's night out, kids' night out, kid night out (apostrophe-stripped), kno, night out, parents night out, ninja night out + synonyms from `gym_valid_values`
- **CLINIC**: clinic + synonyms
- **OPEN GYM**: open gym, play and explore the gym, open to all + synonyms

**Note:** Apostrophe handling — code strips `'`, `'`, and `` ` `` before matching, so "Kid's Night Out", "Kids' Night Out", and "Kids Night Out" all match.
...
# PART 8: WHAT IS NOT VALIDATED

The audit system is a **data cross-reference engine** — it compares structured iClass data against title/description text and pricing tables. It does NOT include:

| Not Validated | Details |
|---------------|---------|
| **Spelling/typos** | No spell check — "Sumemr Camp" would not be flagged |
| **Grammar/punctuation** | No grammar check — missing apostrophes, capitalization issues not caught |
| **Naming consistency across events** | If same gym uses "Kids Night Out" on one event and "Parents Night Out" on another, both pass — no consistency check |
| **Year in DESCRIPTION** | Only checks title for wrong year, not description (known gap) |
| **Consecutive day validation for camps** | Does not verify camps run Mon-Fri or any specific day sequence |
| **Max age** | Only min age is compared — managers often omit max or use "+" notation |
| **`program_ignore` rule type** | Cannot ignore "open gym" when it appears as an activity name inside a KNO description |
| **Day-of-month number in title vs `start_date`** | Engine compares months and years but not day-of-month. So "May 25th" in title for a camp whose `start_date` is May 26 would not be flagged. (Surfaced May 4, 2026 while fixing holiday-week camp dates.) |

---

## Note on date source (May 4, 2026 update)

For camps with a schedule, `start_date` and `end_date` now come from `blocks[0].sqlDate` and `blocks[-1].sqlDate` (the real scheduled meeting days), not from iClass `startDate` / `endDate` (the bookend range). This means `check_date_mismatch`, `check_year_mismatch`, and any other check that consults `start_date` is comparing against the real first day of camp — which is what gym staff write into titles and descriptions. Holiday-week camps (Memorial Day, Labor Day, Thanksgiving, etc.) are now compared against the right date.

---

**This document was generated by reading the actual code in `automation/f12_collect_and_import.py`, `src/lib/validationHelpers.js`, and related components. Every branch and combination documented as implemented. Last verified against code: March 5, 2026 (date source note added May 4, 2026).**
