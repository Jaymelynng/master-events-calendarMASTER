...
export default function AdminAuditErrorCard({
  ...
  const totalActive = activeDataErrors.length + activeStatusErrors.length + (hasDescriptionIssue && !descVerified && !descBug ? 1 : 0);
  ...
  const showDescIssue = hasDescriptionIssue && errorTypeFilter === 'all' && (
      ...
      >
        <div className="flex-1 min-w-0">
          <span className="text-sm">
            {isDismissed ? '✅' : category === 'data_error' ? '🚨' : 'ℹ️'}
            {' '}
