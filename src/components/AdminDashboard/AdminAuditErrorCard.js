...
export default function AdminAuditErrorCard({
  ...
  // Only show DATA errors (FORMAT checks removed to focus on wrong info that hurts signups)
  ...
  const totalActive = activeDataErrors.length + activeStatusErrors.length;
  const totalDismissed = dismissedDataErrors.length + dismissedStatusErrors.length;
  const totalVisible = visibleDataErrors.length + visibleStatusErrors.length;
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="px-4 py-3 space-y-1">
        {/* Data Errors */}
        {renderSection('Data Errors (Wrong Info):', 'DATA', visibleDataErrors, 'bg-red-500', 'text-red-700')}
